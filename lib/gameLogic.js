...
function validateNumPlayers(numPlayers, { min = 3, max = 15 } = {}) {
...
/**
 * Player-supplied display name: strip control chars, collapse whitespace, cap
 * length. Prevents abuse and, defensively, keeps stored names benign in case
 * they're ever rendered (renderers must still escape).
 */
function sanitizePlayerName(name, cap = 40) {
...
/**
 * Parse the LLM word-list response (one word/phrase per line, with various
 * stray punctuation and numbering models sometimes emit). Provider-agnostic.
 */
